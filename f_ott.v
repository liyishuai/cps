...
Definition i := var. (*r integer literals *)
...
Inductive u : Set :=  (*r terms *)
 ...
 | u_int_b (_:nat)
 | u_int_f (i5:i)
 | u_fix (x_5:x) (x1:x) (t1:t) (t2:t) (e:u)
 | u_app (e1:u) (e2:u)
 | u_tlam (e:u)
 | u_tapp (e:u) (t5:t)
 | u_pair (e1:u) (e2:u)
...
Fixpoint open_u_wrt_u_rec (k:nat) (e_5:u) (e__6:u) {struct e__6}: u :=
  ...
  | (u_int_b nat) => 
      match lt_eq_lt_dec nat k with
        | inleft (left _) => u_int_b nat
        | inleft (right _) => e_5
        | inright _ => u_int_b (nat - 1)
      end
  | (u_int_f i5) => u_int_f i5
  | (u_fix x_5 x1 t1 t2 e) => u_fix x_5 x1 t1 t2 (open_u_wrt_u_rec k e_5 e)
...
Fixpoint open_u_wrt_t_rec (k:nat) (t_6:t) (e_5:u) {struct e_5}: u :=
  ...
  | (u_var_b nat) => u_var_b nat
  | (u_var_f x5) => u_var_f x5
  | (u_int_b nat) => u_int_b nat
  | (u_int_f i5) => u_int_f i5
  | (u_fix x_5 x1 t1 t2 e) => u_fix x_5 x1 (open_t_wrt_t_rec k t_6 t1) (open_t_wrt_t_rec k t_6 t2) (open_u_wrt_t_rec k t_6 e)
...
Inductive lc_u : u -> Prop :=    (* defn lc_u *)
 | lc_u_var_f : forall (x5:x),
     (lc_u (u_var_f x5))
 | lc_u_int_f : forall (i5:i),
     (lc_u (u_int_f i5))
 | lc_u_fix : forall (x_5 x1:x) (t1 t2:t) (e:u),
     (lc_t t1) ->
     (lc_t t2) ->
     (lc_u e) ->
     (lc_u (u_fix x_5 x1 t1 t2 e))
...
Fixpoint tt_fv_u (e_5:u) : vars :=
  ...
  | (u_int_b nat) => {}
  | (u_int_f i5) => {}
  | (u_fix x_5 x1 t1 t2 e) => (tt_fv_t t1) \u (tt_fv_t t2) \u (tt_fv_u e)
...
Fixpoint e_subst_u (e_5:u) (x__6:x) (e__6:u) {struct e__6} : u :=
  match e__6 with
  | (u_var_b nat) => u_var_b nat
  | (u_var_f x5) => (if eq_var x5 x__6 then e_5 else (u_var_f x5))
  | (u_int_b nat) => u_int_b nat
  | (u_int_f i5) => u_int_f i5
  | (u_fix x_5 x1 t1 t2 e) => u_fix x_5 x1 t1 t2 (e_subst_u e_5 x__6 e)
  | (u_app e1 e2) => u_app (e_subst_u e_5 x__6 e1) (e_subst_u e_5 x__6 e2)
  | (u_tlam e) => u_tlam (e_subst_u e_5 x__6 e)
  | (u_tapp e t5) => u_tapp (e_subst_u e_5 x__6 e) t5
  | (u_pair e1 e2) => u_pair (e_subst_u e_5 x__6 e1) (e_subst_u e_5 x__6 e2)
  | (u_prl e) => u_prl (e_subst_u e_5 x__6 e)
  | (u_prr e) => u_prr (e_subst_u e_5 x__6 e)
  | (u_prim e1 p5 e2) => u_prim (e_subst_u e_5 x__6 e1) p5 (e_subst_u e_5 x__6 e2)
  | (u_if0 e1 e2 e3) => u_if0 (e_subst_u e_5 x__6 e1) (e_subst_u e_5 x__6 e2) (e_subst_u e_5 x__6 e3)
  | (u_ann u5 t5) => u_ann (e_subst_u e_5 x__6 u5) t5
end.
...
Fixpoint t_subst_u (t_6:t) (a_6:a) (e_5:u) {struct e_5} : u :=
  ...
  | (u_int_b nat) => u_int_b nat
  | (u_int_f i5) => u_int_f i5
  | (u_fix x_5 x1 t1 t2 e) => u_fix x_5 x1 (t_subst_t t_6 a_6 t1) (t_subst_t t_6 a_6 t2) (t_subst_u t_6 a_6 e)
...
Inductive term : D -> G -> u -> t -> Prop :=    (* defn term *)
 ...
 | term_int : forall (D5:D) (G5:G) (i5:i),
     term D5 G5 (u_int_f i5) t_int
 | term_fix : forall (D5:D) (G5:G) (x_5 x1:x) (t1 t2:t) (u5:u),
     type D5 t1 ->
     type D5 t2 ->
     term D5  (( x1 ,  t1 ) ::   (( x_5 ,  (t_arr t1 t2) ) ::  G5 )  )  u5 t2 ->
     term D5 G5 (u_fix x_5 x1 t1 t2 u5) (t_arr t1 t2)
